...
int BPlusTree::splitInternal(int intBlockNum, InternalEntry internalEntries[]) {
    ...
    // Left block gets internalEntries[0..49]
    for (int i = 0; i < 50; i++)
        leftBlk.setEntry(&internalEntries[i], i);

    // Right block gets internalEntries[51..100] (index 50 goes to parent)
    for (int i = 51; i < 101; i++)
        rightBlk.setEntry(&internalEntries[i], i - 51);

    // Update pblock for ALL children that now belong to rightBlk.
    // rightBlk holds entries[51..100]. Its children are:
    //   lChild of entry[0] in rightBlk  → internalEntries[51].lChild
    //   rChild of entry[i] in rightBlk  → internalEntries[51+i].rChild, for i=0..49
    
    InternalEntry entryBuffer;

    // Handle the leftmost child of rightBlk (lChild of rightBlk's first entry)
    rightBlk.getEntry(&entryBuffer, 0);          // gets internalEntries[51]
    {
        BlockBuffer child(entryBuffer.lChild);
        HeadInfo childHeader;
        child.getHeader(&childHeader);
    ...
    // Handle all rChild entries of rightBlk
    for (int i = 0; i < rightBlkHeader.numEntries; i++) {
        rightBlk.getEntry(&entryBuffer, i);
        BlockBuffer child(entryBuffer.rChild);
        HeadInfo childHeader;
